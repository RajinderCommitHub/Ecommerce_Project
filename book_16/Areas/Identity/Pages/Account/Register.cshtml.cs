    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            Input = new InputModel()
            {
                ...
                RoleList = _roleManager.Roles.Where(r => r.Name != SD.Role_Indiviual ).Where(r => r.Name != SD.Role_Admin).Where(r => r.Name != SD.Role_Company).Select(x => x.Name).Select(rl => new SelectListItem()
                {
                    Text = rl,
                    Value = rl
                })
